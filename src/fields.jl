...
Base.Array(F::PlanarField{T,S,L,A}) where {T,S,L,A<:Array} = parent(F)
Base.Array{T}(F::PlanarField{T,S,L,A}) where {T,S,L,A<:Array{T}} = parent(F)
Base.Array{T,N}(F::PlanarField{T,S,L,A}) where {T,N,S,L,A<:Array{T,N}} = parent(F)
